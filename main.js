...
function create() {
  ...
  // Spawn predators over time
  // Predator spawning is scheduled when a run starts so it doesn't make the
  // start screen unfairly difficult. We'll use a dynamic scheduler that
  // adjusts spawn delay based on elapsed time (difficulty ramps up).
  ...
  bgMusic = {
    isPlaying: false,
    play() { AudioManager.startAmbient(); this.isPlaying = true; },
    stop() { AudioManager.stopAmbient(); this.isPlaying = false; }
  };
  ...
  // On first user gesture start the run (this is also the user gesture
  // that allows WebAudio to start). Init audio context so bgMusic.play()
  // can succeed when called from startRun.
  AudioManager.init();
  ...
  // Initial overlay shown until the first run starts
  showOverlay(this, 'Click or tap to start', 'Collect pollen to score • Avoid predators');
}
...
function update(time, delta) {
  ...
  // simple steering toward moth with speed that ramps up over time.
  const ang = Phaser.Math.Angle.Between(pr.x, pr.y, moth.x, moth.y);
  // compute difficulty from elapsed seconds (0..1 over ~45s)
...
function spawnPredator(scene) {
  ...
  pr.setScale(1.0);
  // baseSpeed is modest at the start; actual speed will be scaled by
  // difficulty over time in the update loop.
...
function handleHit(mothObj, predatorObj) {
  ...
  // show overlay with game over info (keeps a consistent UX for non-running state)
  showOverlay(this, 'Game Over', 'Score: ' + finalScore + '\nClick or tap to play again');
  // clicking will reset the game
...
function startRun(scene) {
  ...
  // begin dynamic predator scheduling
  if (predatorTimer) { try { predatorTimer.remove(false); } catch (e) {} }
  scheduleNextPredator(scene);
  // ensure music starts now that the run has begun (user gesture was the
  // pointerdown that called startRun)
  if (!bgMusic.isPlaying) bgMusic.play();
  // hide any overlay when starting
  hideOverlay(scene);
}
...
// Schedule the next predator spawn with a delay that shortens as difficulty
// increases. Uses a delayedCall stored in predatorTimer so it can be cancelled.
function scheduleNextPredator(scene) {
  // compute difficulty from elapsed time (0..1 over ~45s)
  const elapsed = running ? Math.max(0, (performance.now() - startTime) / 1000) : 0;
  const difficulty = Math.min(1, elapsed / 45);
  // delay interpolates from 2200ms (easy) down to 700ms (hard)
  const delay = 2200 - difficulty * (2200 - 700);
  predatorTimer = scene.time.delayedCall(Math.floor(delay), () => {
    if (running) spawnPredator(scene);
    // schedule again only if the run is still active
    if (running) scheduleNextPredator(scene);
  });
}

function showOverlay(scene, title, subtitle) {
  // if there's already an overlay, update text
  if (overlayContainer) {
